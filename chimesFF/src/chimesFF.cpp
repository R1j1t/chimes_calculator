...
void chimesFF::compute_4B(const vector<double> & dx, const vector<double> & dr, const vector<int> & typ_idxs, vector<double> & force, vector<double> & stress, double & energy, chimes4BTmp &tmp, vector<double> & force_scalar_in)
{
    ...
    for(int coeffs=0; coeffs<variablecoeff; coeffs++)
    {
        for (int i=0; i<npairs; i++)
        {
            ...
            deriv[coeffs][i] = fcut[0] * Tnd[i][ powers[coeffs][i] ] + fcutderiv[0] * Tnd[i][ powers[coeffs][i] ];
        }
    ...
    for(int coeffs=0; coeffs<variablecoeff; coeffs++)
    {
        ...
        force_scalar[0]  = coeff * deriv[coeffs][0] * fcut_5[0] * Tn_ik[powers[coeffs][1]]  * Tn_il[powers[coeffs][2]] * Tn_jk_jl * Tn_kl_5 ;
        force_scalar[1]  = coeff * deriv[coeffs][1] * fcut_5[1] * Tn_ij[powers[coeffs][0]]  * Tn_il[powers[coeffs][2]] * Tn_jk_jl * Tn_kl_5 ;
        force_scalar[2]  = coeff * deriv[coeffs][2] * fcut_5[2] * Tn_ij[powers[coeffs][0]]  * Tn_ik[powers[coeffs][1]] * Tn_jk_jl * Tn_kl_5 ;
        force_scalar[3]  = coeff * deriv[coeffs][3] * fcut_5[3] * Tn_ij_ik_il  * Tn_jl[powers[coeffs][4]] * Tn_kl_5 ;
        force_scalar[4]  = coeff * deriv[coeffs][4] * fcut_5[4] * Tn_ij_ik_il  * Tn_jk[powers[coeffs][3]] * Tn_kl_5 ;
        force_scalar[5]  = coeff * deriv[coeffs][5] * fcut_5[5] * Tn_ij_ik_il * Tn_jk_jl ;
