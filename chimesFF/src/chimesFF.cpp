...
void chimesFF::compute_4B(const vector<double> & dx, const vector<double> & dr, const vector<int> & typ_idxs, vector<double> & force, vector<double> & stress, double & energy, chimes4BTmp &tmp, vector<double> & force_scalar_in)
{
    ...
    for(int coeffs=0; coeffs<variablecoeff; coeffs++)
    {
        ...
        // double coeff = chimes_4b_params[quadidx][coeffs];
    ...
    // cout << "Going inside reduction clause "<< endl;

    #pragma omp parallel for reduction(+:energy)
    // #pragma omp parallel for reduction(+:energy) private(coeff, Tn_ij_ik_il, Tn_jk_jl, Tn_kl_5)
    for(int coeffs=0; coeffs<variablecoeff; coeffs++)
    {
        double deriv[npairs];
        double force_scalar[npairs];
...
//         // Accumulate forces/stresses on/from the ij pair
        ...
        // force[0*CHDIM+0] += force_scalar[0] * dr[0*CHDIM+0] + force_scalar[1] * dr[1*CHDIM+0];
        // force[0*CHDIM+1] += force_scalar[0] * dr[0*CHDIM+1] + force_scalar[1] * dr[1*CHDIM+1];
        // force[0*CHDIM+2] += force_scalar[0] * dr[0*CHDIM+2] + force_scalar[1] * dr[1*CHDIM+2];

        // force[0*CHDIM+0] += force_scalar[0] * dr[0*CHDIM+0] + 0.5 * dr[1*CHDIM+0];
        // force[0*CHDIM+1] += force_scalar[0] * dr[0*CHDIM+1] + 1.5 * dr[1*CHDIM+1];
        // force[0*CHDIM+2] += force_scalar[0] * dr[0*CHDIM+2] + 2.0 * dr[1*CHDIM+2];
...
//         // Accumulate forces/stresses on/from the ik pair
        // cout << "force_scalar[1]" << endl;
        // cout << "force_scalar[1] " << force_scalar[1] << endl;
...
//         // Accumulate forces/stresses on/from the kl pair
    ...
    // cout << "Exiting reduction clause "<< endl;
